...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (event.type.startsWith("payment_intent.")) {
      ...
      // app/api/payments/webhook/route.ts  (only safe-narrowing changed)
      const latestChargeObj = typeof full.latest_charge === "string" ? null : full.latest_charge;
      const card = latestChargeObj?.payment_method_details?.card;
